...
from app.site_cookies import (
    ...
    inspect_site_cookies_file_for_url,
)
...
def resolve_cookies_params(
    *,
    ...
    managed_cookies_inspector_fn=inspect_site_cookies_file_for_url,
    ...
    managed_path, managed_issue = managed_cookies_inspector_fn(url or "")
    if managed_path is not None:
        if managed_issue is None:
            log(f"🍪 Using managed site cookies for URL: {url}")
            return ["--cookies", str(managed_path)]

        log(
            "⚠️ Ignoring invalid managed site cookies file: "
            f"{managed_path} ({managed_issue})"
        )
    else:
